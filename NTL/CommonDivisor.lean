...
2 つの自然数 $a$ と $b$ に対して，そのどちらの約数でもあるような自然数のことを，公約数(common divisor)といいます．言い換えると，$d ∈ ℕ$ が $a$ と $b$ の公約数であるとは，$d ∣ a$ かつ $d ∣ b$ が成り立つことです．

[整除関係の節](./Divisibility.md)で見たように，$1$ はどんな自然数の約数でもあるため，$a$, $b$ の公約数は必ず1つは存在します．かつ，公約数が無限に存在することはありえません．したがって，最大のものが存在します．それを $a$ と $b$ の最大公約数(greatest common divisor)と呼ぶことにして，$\gcd(a, b)$ と書きます．-/
