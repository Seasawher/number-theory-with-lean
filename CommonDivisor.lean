...
[整除関係の節](./Divisibility.md)で見たように，$1$ はどんな自然数の約数でもあるため，$a$, $b$ の公約数は必ず1つは存在します．かつ，公約数が無限に存在することはありえません．したがって，最大のものが存在します．それを $a$ と $b$ の{{i:最大公約数(greatest common divisor)}}と呼ぶことにして，$\gcd(a, b)$ と書きます．-/
