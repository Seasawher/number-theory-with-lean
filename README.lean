...
本書のすべての Lean コードブロックには Lean Playground へジャンプするボタンが搭載されています．しかし必要な `import` 文などが含まれていないため，ほとんどの場合そのままでは動かないと思います．右上にある編集(suggest an edit)ボタン <i class="fa fa-edit"></i> をクリックすると，元の `*.lean` ファイルにジャンプすることができますから，それを Playground にコピーして実行してください．
